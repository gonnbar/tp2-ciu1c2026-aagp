...
export default function Register() {
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    e.preventDefault();
    const resultado = validarRegistro(nickname, email, contraseña);
    ...
    try {
      ...
      if (!respuesta.ok) {
        ...
        if (data.error?.includes("nickname")) {
          ...
          setMensaje(data.message || "No se pudo crear el usuario");
        }
        return;
      }
    ...
    <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8 pt-30">
      <div className="mx-auto w-full max-w-md rounded-xl bg-[var(--surface-soft)] shadow-lg px-6 py-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-sm">
          ...
          <h2 className="mt-6 text-center text-2xl font-bold tracking-tight text-text">
            {" "}
            Crear cuenta{" "}
          </h2>
        </div>
        ...
        <div className="mt-6 sm:mx-auto sm:w-full sm:max-w-sm">
          ...
          >
            <div>
              ...
              <div className="mt-2">
                <input
                  ...
                  className="block w-full rounded-md border border-border bg-surface px-3 py-1.5 text-text outline-none focus:border-primary"
                />
                {errores.nickname && (
            ...
            <div>
              <button
                type="submit"
                className="flex w-full justify-center rounded-lg bg-primary hover:bg-primary-dark px-4 py-2 text-white text-lg cursor-pointer"
              >
                Registrarse{" "}
              </button>
            </div>
